...
***The distribution is skewed to the left, meaning more students give their professors a high score as not as many students give their professors a low score. This does not really surprised me. I didn't know what to expect. It is interesting to see the mean and median so similar to each other.***
```{r}
evals %>%
  ggplot(aes(x = score)) +
  geom_histogram()
...
***The graph shows a positive relationship between the average beauty rating and the average evaluation rating score, but just a slight increase.***
...
evals %>%
  ...
  ylab("Average Proffessor Evaluation") +
  ggtitle("Scatter plot of Average Beauty Rating to Average Evaluation Rating Score")
```
...
***"Jitter" means that the points are more seperated with space rather than in a straight line like `geom_point`. The initial scatterplot is misleading, for you don't know the exact spot each point lands.***
...
***The model is :***
***score = 4.09 + 0.10 • Male***
***A male professor on average scores 0.14 higher on evaluations than a female professor. On average a female professor scores 4.09 on the ratings.***
```{r}
...
```
***The model is:***
***score = 4.28 - 0.12 • Tenure Track - 0.14 • Tenured***
...
score_rank_relevel_fit %>% flextable() %>% autofit()
```
***The model is:***
